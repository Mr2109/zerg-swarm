...
- **多 worker 并发路由**：FleetController 改为按机器分组 worker（fleet-dispatch 分发 → 每机器独立 worker + 独立锁）——跨机器并行已验证（DS4/X3 + ornith/本机同时服务）
- **切换链路**：空→qwen3-32b→DS4→glm-z1→ornith 连续轮换全部 OK（agent 停旧起新 + 端口确认）
