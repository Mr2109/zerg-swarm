//! 虫茧模块系统（v2.5.6 UI 蓝图——Mr2109 2026-08-29）
//!
//! 虫族 UI = 虫茧船：
//! - 船体（核心）: 任务体系（任务队列/内部任务）+ 基础设施（集群/模型库/资源库）——永驻
//! - 虫茧（模块）: 文档/Git/日志 + 未来生态应用——可吊装可卸下
//!
//! 模块注册表 + 顶部导航渲染入口。
...
pub fn build_registry() -> ModuleRegistry {
    ...
    // 🧩 可装卸箱（甲板箱——默认在船）
    // 文件浏览器（阶段 1——2026-09-13 设计「文件浏览器虫茧」§4.1）：只是一只**薄壳箱**，
...
///       父箱下再一行二级页签（集群/文件浏览器/升级/Git/日志 ｜ 任务队列/内部任务 ｜ 模型库/模型登记库）
...
/// - ➕ = 模块管理入口（装卸箱——含虫茧平台里的应用）；右侧固定: 语言切换 + HUD 开关 + 登录用户
pub fn top_nav_bar(
    ui: &mut egui::Ui,
