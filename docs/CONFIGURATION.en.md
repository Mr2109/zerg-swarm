...
<model-name>:
  ...
  chat_template: ~/.zerg/ornith_chat_template.jinja   # optional (~ is supported)
```

`chat_template` resolution priority: **registry field → `ZERG_ORNITH_TEMPLATE` → `~/.zerg/ornith_chat_template.jinja` → a relative path inside the repository**;
