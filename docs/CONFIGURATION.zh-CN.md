...
> *中文原版（唯一真相源）。英文版（[CONFIGURATION.en.md](CONFIGURATION.en.md)）为派生翻译；两版如有不一致，以**本文**为准。*
...
除了令牌与登记表，多机部署还有一层**部署事实**：每台机器属于哪一档种类。

| 档 | 含义 | 选机行为 |
|---|---|---|
| `ai` | AI 专用机（如大显存统一内存机器） | 默认承接；最优先 |
| `mini` | 小型机（内容小、装得下才接） | 只接**能力硬门**装得下的请求 |
| `work` | 工作机（自己也做主人的活） | 最后才用，且**可被抢占让位** |

- **溢出序**：`ai` → `mini` → `work`。档位更优的候选在场时，低档不抢。
- **真源**是一份部署侧的上下文件（`~/.zerg/contexts/default.yaml` 的 `kind`），主控侧的舰队投影由它生成；**没写种类时按 `work` 处理**（宁可少用，也不误占 AI 机）。
- 机器种类是**部署事实**，不写进控制层规则文件——控制层与调度策略是两回事。
