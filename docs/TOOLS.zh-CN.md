...
> *中文原版（唯一真相源）。英文版（[TOOLS.en.md](TOOLS.en.md)）为派生翻译；两版如有不一致，以**本文**为准。*
...
公开面导出与预检各有入口，命令行里就能走完：

| 入口 | 作用 | 性质 |
|---|---|---|
| `zerg publish run` | 在本地产物目录生成公开镜像树（白名单 + 排除项 + 脱敏替换，压成单个快照提交） | 危险动作（需三态确认） |
| `zerg publish preflight <产物目录>` | 只读预检：各道门禁 + 现值面 + 命令名对拍 + 生成区一致性 | **只读**（不改任何状态） |
| `zerg publish tree has` | 查某件是否在某棵产出树里 | 只读 |
| `zerg script ls` | 列出脚本面（含发布三条脚本） | 只读 |

预检**必须带产物目录参数**；预检不过 ⇒ 不得推送。
