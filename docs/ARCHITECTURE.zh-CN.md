...
**会话粘性**（补齐「键」）：多机部署里，同一个会话如果每轮都重新选机，会被劈到不同机器上——上下文各半、体感极差。现在的口径是：

1. **显式请求头**（`X-Zerg-Session`）——最强的键，调用方自己给；
2. **请求体里的会话号**——OpenAI 兼容面天然的键；
3. **隐式键**——由「模型 id + 规范化后的系统提示」推导，同一路调用方不显式给也能落到同一台。

选机与**机器种类档序**（`ai` → `mini` → `work`）叠加：更优档的候选在场时不降档；档位槽满按「忙不可用」处理，工作机最后才承接。

**故障必须可见**：粘性绑定只在**转发成功**的那条路径上回写；失败、熔断、诊断都走既有回路（熔断标记 + 诊断行），**不会**被伪装成「换台重试」。
