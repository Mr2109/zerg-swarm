...
Code generated by scripts/docs/gen-cli-reference.py — DO NOT EDIT. **本块为生成物（勿手改）**：由 `scripts/docs/gen-cli-reference.py` 照命令树**现算**后注入 —— 重跑 `python3 scripts/docs/gen-cli-reference.py --emit --target publish/docs/CLI.zh-CN.md`；判 drift 用同名 `--check --target publish/docs/CLI.zh-CN.md`。本块里的命令名、用法串（旗标）、说明、`--json` 字段、投影端点与茧壁层级**逐字来自命令树，一个都不是手写的**。
...
> 导出物抬头逐字：`命令清单 **90** 条 · 危险动作 **40** 条（其中**已开放** 12 条 · 未开放 28 条）` —— 「哪些已开放」逐条以 `zerg help dangerous` 为准；本表四列（档 / 三态 / `--confirm` 的目标 / 它会动什么）逐字来自 `zerg help export`；未开放的那部分真跑一律**拒执**（退出码 2 = 不给结论），只有 `--dry-run` 的计划面可用。
...
- **拒绝规则**：请求未知字段 ⇒ 退码 `2` + 列全部合法字段；`--json` 不给字段 ⇒ 退码 `2` + stderr 列全部字段（schema 自描述）；`--schema` 主号不认 ⇒ 退码 `2`；`--json '*'` **不存在** —— 要的是逗号分隔的字段名。
