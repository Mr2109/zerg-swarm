...
> *中文原版（唯一真相源）。英文版（[README.md](README.md)）为派生翻译；两版如有不一致，以**本文**为准。*
...
- **命令面**：常用动作都有唯一命令入口 —— 热加载配置（`zerg config reload`，改完即刻生效、不重启主控）；把模型登记进名册（`zerg model add`）；卵面的只读投影（`zerg egg ls` / `zerg egg show`，卵 × 设备矩阵的 host / model / state 三格）
...
- **发布流水线接进 CLI**：`zerg publish preflight <产物目录>`（只读预检）与 `zerg publish run`（在本地产物目录生成公开镜像树；带三态确认，**不出网、不推送**）。
- **机器种类三档**：`ai`（AI 专用机）→ `mini`（小型机）→ `work`（工作机）。按能力硬门与溢出序选机，工作机最后才用、并会被抢占让位。
- **会话粘性有了键**：同一会话优先落回同一台机器（显式请求头 → 请求体会话号 → 由模型与系统提示推导的隐式键），避免同一会话在两台机器之间来回劈。
- 命令面现为 **129 条**：89 条开放 + 40 条危险动作（需显式确认）。
