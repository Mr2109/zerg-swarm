...
// 真实依据：主线 llama.cpp 不支持 k2-horizon 架构（unknown model architecture），
...
func TestNonMainline_K2AdapterDeclares(t *testing.T) {
	adp := modeladapter.Dispatch("K2-Horizon-Test")
...
func TestNonMainline_MissingCmdRejected(t *testing.T) {
	...
	resp, err := m.doStart("K2-Horizon-Test", entry)
	if err != nil {
	...
	if _, exists := m.procs["K2-Horizon-Test"]; exists {
