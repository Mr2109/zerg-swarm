...
func TestDispatchK2Horizon(t *testing.T) {
	cases := []struct {
		...
		{"example-moe-36b", "example-moe-36b"},
		{"example-moe-36b-mova-36b-a4b", "example-moe-36b"},
		{"example-moe-36b-7B", "example-moe-36b"},
		{"example-moe-36b-375B-A23B", "example-moe-36b"},
		// 不误伤其他模型
		{"Qwen3.8-Flash-Next", "qwen3.8-flash"},
		{"example-35b-v2", "example-35b-v2"},
		{"deepseek-v4-flash", "deepseek"},
		{"不认识的模型", ""}, // generic
	}
	// 公开快照会把**本文件的用例名**替换成示例名（example-moe-36b-* → example-*），
	// "按名字派发"的断言前提随之不成立 → 跳过（本机/私有仓照常执行，覆盖不丢）。
	// 判据必须看"用例表自己"：派发代码本身不会被改写，拿它探测判断不出快照形态。
	if !strings.Contains(cases[0].model, "example-moe-36b") {
