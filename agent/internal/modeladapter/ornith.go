...
// 特性（调研结论 docs/调研-ornith工具调用根因.md）：
...
func (a *Ornith) Name() string { return "ornith" }
...
func (a *Ornith) BuildArgs(entry *registry.ModelEntry, port int) []string {
	...
	// D3 多模态：视觉投影（ornith-1.5 是 qwen35moe-vlm——agent_models.yaml mmproj 字段）
	if entry.MMProj != "" {
...
// templatePath 解析 ornith 外部 chat template 路径（可选；都不存在时不加该参数）。
