...
// 不受「本机恰好存在 ~/.zerg/example-35b-v2_chat_template.jinja」这类环境因素影响。
...
// 4) ~ 展开 + 家目录约定路径（~/.zerg/example-35b-v2_chat_template.jinja）。
func TestOrnithTemplateTildeAndHomeConvention(t *testing.T) {
	...
	// 4b) 无任何配置 → 家目录约定路径命中
	convention := writeTemplate(t, filepath.Join(home, ".zerg"), "example-35b-v2_chat_template.jinja")
...
func TestRegistryParsesChatTemplate(t *testing.T) {
	...
	content := "example-35b-v2:\n" +
		...
		"  chat_template: ~/.zerg/example-35b-v2_chat_template.jinja\n"
	...
	if e.ChatTemplate != "~/.zerg/example-35b-v2_chat_template.jinja" {
