...
# 任务表：`Zerg-内部文档/项目文档/v2.5.9/任务单-茧壁-20260916.md` §3（2'.6）（2026-09-19 二次分家）
#
...
#   bash scripts/build/build-wall.sh --debug    # debug   → wall/target/debug/zerg-wall（判据 7 的门禁脚本读它）
#
# 签名：落 `bin/` 的制品由 `scripts/build/build-all.sh` 的签名环节**统一**施加（稳定身份 + 固定 identifier）
...
for arg in "$@"; do
  case "${arg}" in
    --debug) PROFILE=debug ;;
    --help|-h)
      echo "用法: bash scripts/build/build-wall.sh [--debug]"
