...
#
# 用法：
#   bash scripts/build/start-zerg-core.sh          # 已托管则重启（kickstart -k），未托管则安装并启动
#   bash scripts/build/start-zerg-core.sh --stop   # 停止并卸载托管（临时维护用）
#   bash scripts/build/start-zerg-core.sh --status # 查看托管状态
#
# 环境变量来源：仓库根 .env（由 scripts/svc/zerg-core-daemon.sh 承载，launchd 调它）。
set -euo pipefail
...
<dict>
	...
	<array><string>/bin/bash</string><string>${REPO_ROOT}/scripts/svc/zerg-core-daemon.sh</string></array>
...
case "${1:-}" in
  ...
  --stop)
    ...
    echo "✓ 已停止并卸载托管（重启托管：bash scripts/build/start-zerg-core.sh）"
    exit 0
    ;;
esac

chmod +x "$REPO_ROOT/scripts/svc/zerg-core-daemon.sh" 2>/dev/null || true
