...
2. **本门的结论落账面**：`docs/项目文档/*/债务台账-*`（glob）——
   **记录不能充作引用**。台账点名一个死件不会让它活过来；反过来，若把记录算作引用，
   ...
   ★ **2026-09-19「项目文档」二次分家批2（只加注、条目值一字不动）**：本条今天**仍在咬**（仓内现跑命中
   `docs/项目文档/v2.5.10/债务台账-20260918.md` / `.tsv` 两件）；该目录随**批3** 移出工作树后本条
   **仓内零命中** ⇒ 届时删或随分家改指仓外（`../Zerg-内部文档/项目文档/*/债务台账-*`）——**改指要连同匹配口径
   一起改**：本门只列**仓内**相对路径（`git ls-files` / 文件系统遍历），`../` 形态的条目前置进不来，
   直接写仓外 glob = 空条目（另一种假覆盖）。本批**不加仓外条目**。
3. 候选脚本**自身**那一个文件。
