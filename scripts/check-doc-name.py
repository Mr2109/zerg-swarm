...
#     删除（死条目留着 = 假覆盖）。本批不删：对象今天仍在仓内、仍有命中在咬（`--scope repo` 现跑
#     冻结区 N7 126 条 / N5 混用等）——现在删 = 拿掉一道真在咬的判据；也不改指仓外：本门只扫**仓内**
#     路径，仓外那棵不进货架 ⇒ 指过去是空条目（另一种假覆盖）。
...
EXEMPT_A = [
    ...
    #   ★ 2026-09-19 「项目文档」二次分家**批2**（**只加注、A14/A15/A16 三条一字不动**）：这三条的
    #     豁免对象都在 `docs/项目文档/` 里（仓内今天仍在 ⇒ 现跑仍各有命中：A15 ×17 · A16 ×1；
    #     A14 的六目录今天零命中，见下方口径）。**批3** 把该目录移出工作树后三条**仓内零命中**
    #     ⇒ 届时按「零命中即删」删除（死条目留着 = 假覆盖）。本批不删：现在删只会把仍存在的
    #     冻结区告警从「已豁免」挪回「冻结区只登记」（rc 不变、但豁免面假缩水）；也不改指仓外：
    #     `prefix` / `prefix_name` 形态匹配的是**扫到的仓内相对路径**，仓外那棵不进货架 ⇒ 指过去 = 空条目。
...
GOOD_FILES = {
    ...
    #   ★ 2026-09-19 「项目文档」二次分家批2：本条是**自检夹具里的合成路径**（临时树），不是真仓对象
    #     ⇒ 随批3 搬移**不必改**（冻结区/豁免域判据在临时树上照旧各自有牙；改指仓外反而会造空夹具）。
