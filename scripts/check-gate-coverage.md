...
> **规格（真源，只读）**
> `Zerg-内部文档/项目文档/v2.5.10/债务台账-20260918.md` **§0**（「哪些域在所有门之外」11 行）与 **§7** 建议门①；
> 机读件 `Zerg-内部文档/项目文档/v2.5.10/债务台账-20260918.tsv`。
> ★ 2026-09-19 「项目文档」二次分家批5：台账两件（`.md` / `.tsv`）随该目录整目录移出工作树
> ⇒ 上面的出处路径已改指仓外取源根；本门**判据值一个字没动**（这里只改措辞，脚本不读该件内容）。
