...
设计取舍（三条，都是被咬过的）
  ...
  ② **schema 与判定同源**：schema 里出现本脚本未实现的关键字 ⇒ **rc=2**（否则 schema 加了规则、
     门还是绿的 ⇒ 假覆盖）。
  ③ **不算不合规的类别要显式剔出并计数**：`SKILL.md`（Hermes/技能格式是**另一套** frontmatter
     契约：只有 name/description，不带文档 6 件套）⇒ 剔除不是隐藏，报告里逐类给数。
     ★ 原第一类「`docs/调研/multi-agent-源码/`（第三方源码摘录）」**已于 2026-09-19 随源树从主仓
     删除而删除**：该树先 `cp -a` 到 `Zerg-内部文档/调研/multi-agent-源码/`，同日按用户令从主仓移除
     ⇒ 仓内**零命中** ⇒ 死条目删除（口径：零命中即删；死条目留着 = 假覆盖）。
     ★ 反向探针（同日 · /tmp 仿真造树 `…/x/SKILL.md` + `…/y/README-20260901.md`）：
     **条目在** ⇒ 扫描域 **1** 篇 / 合规 1 / 不合规 **0**（rc=0）；**条目删** ⇒ 扫描域 **3** 篇 /
     不合规 **2**（M2 六件套缺项，rc=1）⇒ 条目确在起作用。真仓侧该树已不在盘上 ⇒ 删条目前后
     `--scope repo/formal` 的扫描域与各类计数**逐位一致**（无对象可剔 ⇒ 既不加红也不生假绿）。
...
#   ★ 2026-09-19 分家第二步（源树从主仓删除）：`docs/调研/multi-agent-源码/` 已 `cp -a` 到
#     `Zerg-内部文档/调研/`，同日按用户令**从主仓移除**（主仓盘上已不存在）⇒ 本条**仓内零命中 ⇒
#     删除**（口径：零命中即删；死条目留着 = 假覆盖）。反向探针见文件头 ③：造树 ⇒ 条目在
