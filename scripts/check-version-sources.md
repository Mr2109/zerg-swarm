...
> **状态：新增件（2026-09-18，门② 交付；挂接未做 —— 由父代理决定何时进 `scripts/precommit-gates.sh`）**。
...
| 挂法（建议 · 未执行） | 进 `precommit-gates.sh` 步骤表，模式 **`tri`**（亚秒级只读，无外部依赖）；rc=2 记 BLOCKED、不计失败项 |
...
- **挂法**：进 `scripts/precommit-gates.sh` 的**步骤表**（一等步骤，不是脚本尾部软检查位），
...
- **承接项**：① 挂接（父代理）② D1 是否转阻断档 + 门槛（待 Mr2109 拍）③ `wall/Cargo.lock` 是否纳入判据（待拍）
