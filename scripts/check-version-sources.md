...
| 排除域 | 理由 |
|---|---|
| `Zerg-内部文档/`（**2026-09-19 分家**：开发文档分家在**工作树之外**的同级目录；原 `docs/02-调研/raw/` 与 `docs/issues/` 两条已删） | 设计稿/调研稿/评审稿/问题单整树 —— 仓内若出现同名路径（搬回 / 软链 / 副本），其版本号属归档料，只登记不判 |
| `docs/项目文档/v<X>/`（X ≠ 当前版；**293 件**） | 他版快照目录（17 套历史快照，冻结归档） |
| `docs/项目文档/v<当前版>/` | **不排除** —— 它是活文档，命中要进清单 |
| `docs/.*` · `*/.hidden` | 工具/隐藏目录（如 `docs/.obsidian/`） |

> ★ **2026-09-19「项目文档」二次分家批2（只加注、两条一字不动）**：上面 `docs/项目文档/…` 那两行的**域**
> 随**批3** 移出工作树 ⇒ 本门语料（仓内 `docs/**`）里**零命中** ⇒ 届时按「零命中即删」删两条并按仓外口径改述。
> 本批不删（对象今天仍在仓内、仍在咬：现跑清单里「当前 2.5.9 = 陈旧陈述」那类正是它兜住的）。
> ★ 同批**已改**：**旧版集合的取源根双认** —— 见下。

> ★ **2026-09-19 变更**：原第一行 `docs/虫族文档/`（1981 件 · 镜像副本 · 公开面镜像，冻结）**已随镜像退役**
> —— 条目从 `DOC_EXCLUDES_FIXED` **删除**（它今天匹配 0 个文件 ⇒ 删除不改任何计数：语料/清单逐位一致，
> 自检⑯ 的「排除域零命中」仍由上面三个域咬住）。镜像若从备份重建，请把该条加回。

**排除域只影响 D1（只报告）那一段**；四处版本源的断言不受任何排除域影响。
**旧版集合**默认 = **直接前驱版** = 快照目录里已知的 3 段版本号中 < 当前版的最大者
（今天 `{2.0.0, 2.5.0…2.5.10}` ⇒ 旧版 = **`2.5.9`**）；`--stale` / `--stale-all` 可覆盖。
> ★ **2026-09-19 二次分家批2（双认 · 先仓内、缺则仓外取源根 · 缺件 rc=2）**：**快照目录取源根** =
> 仓内 `docs/项目文档` 优先，缺则同级仓外 `../Zerg-内部文档/项目文档`（与 `docs/site/export-and-build.sh`
> 的 `ZERG_DOCS_ALT` **同根同默认**）。**两处都不在盘上 ⇒ `rc=2` 不给结论**（原先会静默报
> 「没有历史版本目录可比」= 假绿）。反向探针（同日 · /tmp 仿真）：仓内无、仓外有 ⇒ 旧版集合照得 `2.5.9`
> （`--list-rules` 现跑打印「取源根 = 仓内 docs/项目文档」）；两处都缺 ⇒ `rc=2` + 逐条打印缺件路径。
> **四处版本源断言与所有排除域判据一字未改**（只加了取源根解析与缺件闸）。
