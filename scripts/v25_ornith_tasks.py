#!/usr/bin/env python3
"""v25_ornith_tasks.py — v2.5 Agent 大量任务测试（ornith 主模型——2026-08-13）
...
with open(os.path.join(WORK, "data.txt"), "w") as f:
    f.write("hello-ornith-42")
out = run_agent("读取 data.txt 内容并汇报", WORK)
check("读文件", "hello-ornith-42" in out)
