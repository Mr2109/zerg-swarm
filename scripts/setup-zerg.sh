...
#              语义同 scripts/check-gotoolchain.py）；不合就清楚招供 + 给安装指引，**绝不自行升级**
#   ③ 构建     按角色选组件集，走**仓库自己的入口** scripts/build-all.sh
...
#              .env/PATH 由仓库自带的 scripts/zerg-core-daemon.sh 承载）
...
#
# 用法：
#   bash scripts/setup-zerg.sh                         # 主控机（macOS）默认路径
#   bash scripts/setup-zerg.sh --role node             # 机群节点（linux，systemd 管 agentd）
#   bash scripts/setup-zerg.sh --dir ~/zerg-swarm      # 用已有检出（脏树会被拒绝，不会静默覆盖）
#   bash scripts/setup-zerg.sh --repo <url> --ref main # 换公开镜像仓 / 分支
#   bash scripts/setup-zerg.sh --prefix /opt/zerg/bin  # 换安装前缀（默认 <树>/bin）
...
check_toolchain() {
  ...
  if [ -f "$TREE/scripts/check-gotoolchain.py" ]; then
    python3 "$TREE/scripts/check-gotoolchain.py" "$TREE" >/dev/null 2>&1 \
      || die 'core/agent 两模块的工具链钉住不一致（scripts/check-gotoolchain.py 未过）' 7
...
build_components() {
  local script="$TREE/scripts/build-all.sh" args
  args=()
  [ -f "$script" ] || die "源码树缺 scripts/build-all.sh —— 拒绝用手写 go build 顶替（构建必须走仓库自己的入口）" 5
  if ! has_comp ui "$COMP_LIST"; then args=(--no-ui); fi
  say "🔨 构建（bash scripts/build-all.sh${args:+ ${args[*]}}：身份注入 -ldflags + ad-hoc 重签）"
  ( cd "$TREE" && bash scripts/build-all.sh ${args[@]+"${args[@]}"} ) || die '构建失败（scripts/build-all.sh 退出非零）' 1
}
...
DAEMON="$TREE/scripts/zerg-core-daemon.sh"
write_core_plist() {
...
if [ "$DRY" = 1 ]; then
  ...
  say "   构建     : bash ${TREE}/scripts/build-all.sh$([ "$NO_UI" = 1 ] && echo ' --no-ui')"
...
if [ "$ROLE" = node ]; then
  say "   下一步：回到主控跑 \`bash scripts/zerg-upgrade.sh --fleet\`（该机已可自更新）"
else
