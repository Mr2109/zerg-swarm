...
# scripts/build/start-zerg-core.sh 拉起（与托管路径同一套环境），因此与 launchd 同时存在也不会冲突。
# 用法: nohup bash scripts/svc/watchdog.sh > /tmp/zerg-watchdog.log 2>&1 &
...
while true; do
    ...
    if [ -z "$PID" ]; then
        ...
        # 2026-09-11：统一走 scripts/build/start-zerg-core.sh（setsid 分离 + 源仓库根 .env），
        # 保证「看门狗拉起」与「手动启动」的环境完全一致（旧写法 cd /tmp 且不源 .env → 丢白名单/知识库路径）。
        log "🔄 拉起主控（scripts/build/start-zerg-core.sh）..."
        REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
        if bash "$REPO_ROOT/scripts/build/start-zerg-core.sh" >>"$WD_LOG" 2>&1; then
