...
RECORD_SURFACES = [
    #   ★ 2026-09-19 「项目文档」二次分家**批2**（**只加注、条目值一字不动**）：本条今天**仍在咬**
    #     （仓内现跑命中 2 件：`docs/项目文档/v2.5.10/债务台账-20260918.md` / `.tsv`）。
    #     **批3** 把 `docs/项目文档/` 移出工作树后本条**仓内零命中** ⇒ 届时按「零命中即删」删除，
    #     或随分家改指仓外（`../Zerg-内部文档/项目文档/*/债务台账-*`）—— 注意**改指要连同匹配口径一起改**：
    #     本门只列**仓内**（文件系统遍历 / git 目录）相对路径，`../` 形态的条目前置进不来，
    #     直接写仓外 glob = 空条目（另一种假覆盖）。本批**不加仓外条目**：留一条真有牙的比挂一条死条强。
