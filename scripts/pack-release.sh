...
#
# 用法：
#   bash scripts/pack-release.sh              # 构建 + 打包（含 UI，约 4 分钟）
#   bash scripts/pack-release.sh --no-build   # 复用 dist/<版本>/ 里已构建的二进制，只重打包
#   bash scripts/pack-release.sh --verify     # 只校验：manifest + 逐件 sha256 必须与文件一致
#
