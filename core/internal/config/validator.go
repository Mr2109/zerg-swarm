...
var KnownFamilies = []string{
	...
	"example-35b-v2", "cogvlm", "internlm", "yi", "baichuan", "chatglm",
...
//   - example-moe-36b：agent/internal/modeladapter/k2horizon.go 文件头（llama.cpp IFM fork 专用，
//     主线未支持 issue#28361）；
//   - 设计-子端沙箱化-20260914 §1.2（引擎实现/变体是卵的必需字段）、§4.7（卵清单必须带上它，
//     集群级统一）、附录 C·C1（fleet.yaml 全篇 0 处 cmd: ⇒ 第二台设备孵 K2 会静默退回主线）。
var MainlineUnsupportedArchitectures = []string{"example-moe-36b"}
...
func Validate(name string, candidate ModelCandidate) ValidationResult {
	...
	// 起不来（主线报 unknown model architecture: example-moe-36b）或误链，**而且不报错**
