...
// 主线 llama-server ⇒ 起不来（unknown model architecture: example-moe-36b）或误链，**且不报错**。
...
func TestEngineImpl_MissingCmdIsFatal(t *testing.T) {
	...
	for _, want := range []string{"静默", "主线", "example-moe-36b", "cmd"} {
...
models:
  example-moe-36b:
    - { host: x3, backend: llama-server, file: /data/models/k2/x.gguf, mem_gb: 23, architecture: example-moe-36b, cmd: ["/home/g01/agent/run-k2.sh", "-m", "{file}", "--port", "{port}"] }
  Block-Form-Fork:
    - host: x3
      backend: llama-server
