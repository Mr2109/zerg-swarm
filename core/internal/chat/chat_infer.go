...
func (c *ChatInfer) InferStream(ctx context.Context, model string, sysPrompt string, msgs []map[string]any,
	...
	for scanner.Scan() {
		...
		if d.Content != "" {
			...
			if onDelta != nil {
				// P4-45 XML 工具调用不推正文（ornith 训练格式——content 流式是 XML 标签）
	...
	// ornith 训练格式 = XML 工具调用（模板要求 <tool_call><function=X><parameter=K>V</parameter></function></tool_call>）
...
func parseChatResultTolerant(raw []byte) *InferResult {
	...
	// P4-45 XML fallback（ornith 训练格式——content 里 <tool_call>）
	if len(res.ToolCalls) == 0 && strings.Contains(res.Content, "<tool_call>") {
