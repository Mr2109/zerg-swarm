...
//	· fleet  = 机群**资产下发**形态（无检出/无工具链，件由主控推过来）——拒绝自更新
//	· unknown= 非 git 安装（发行版包/手工放置）——拒绝并给指引
//
// ⚠️ B5（2026-09-13）之后语义收窄：机群节点**默认走 git 形态**——每台机器自己有检出、
// 各自 `zerg update`（fetch→本机构建→交本机六阶段内核），主控只编排与核对版本矩阵。
// 于是 `fleet` 印记只剩"这台机器没有检出、只能由主控下发资产"这一种含义（应急/受限节点）。
//
