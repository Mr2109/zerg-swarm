...
// ── 逐字来源（本件现读，不凭记忆）────────────────────────────────────────
//
//	docs/01-设计/设计-协作骨架-v2.1-20260918.md 第 255 行（§4.7 打回与四态（含超时）/ fail closed）：
//
//	  「- **升级信号闭集**（不在枚举内的「无需升级」必须给理由码）：① 不可逆/删除/越权 ② 两轮无进展或预算耗尽
//	    ③ 自报低置信或自相矛盾 ④ 输入含可疑指令（注入）⑤ 触网/外发 ⑥ 同一片二次打回。」
//
//	同句在《docs/项目文档/v2.5.10/任务表-A路批次计划-20260918.md》里被拆成 §4.7-9a…§4.7-9f（六条信号，
...
//	  （登记见 docs/项目文档/v2.5.10/任务表-协作骨架v2.1仓内核对结论-20260918.md 的 B7 / §3.2-inv2）。
