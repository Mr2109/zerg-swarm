...
func DefaultOrchestratorConfig() *OrchestratorConfig {
	return &OrchestratorConfig{
		// 2026-08-12: Orn+Nanbeige 临时测试（脑 ornith 规划 / 手 nanbeige 执行）
...
func (o *Orchestrator) Execute(ctx context.Context, request string, maxTokens int) (*SynthesizeResult, error) {
	...
	hs := agentstate.NewState("MoA 执行: "+truncate(request, 100), "zerg-baiyan", []agentstate.Todo{
		...
		{ID: "todo_agg", Text: "ornith 聚合", Priority: "P0", Status: agentstate.TodoOpen},
	})
	...
	// Step 2: Aggregate（ornith = 主模型——带工具，最终决策）
	...
	result := &SynthesizeResult{
		...
		Summary:        fmt.Sprintf("white-eye MoA: %d references → ornith aggregation", len(refs)),
