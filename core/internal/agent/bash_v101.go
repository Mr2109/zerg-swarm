...
// 设计: docs/01-设计/设计-bash工具v1.0.1-AI专用执行契约-20260906.md
		...
		for i := 0; i < len(fields); i++ {
			...
			for j := i + 1; j < len(fields); j++ {
				...
				if !rmInAllowedRoot(cleaned, roots) {
					return fmt.Errorf("危险命令拦截: rm 目标 %q 不在允许删除域(工作区//tmp/白名单)——v1.0.2 删除范围门控。请改工作区内路径或用专用文件工具;确需他处删除请加白名单或人工执行(设计: docs/01-设计/设计-bash工具v1.0.2-删除范围门控-20260907.md)", target)
				}
			}
		}
	}
