...
func TestParseLlamaCmdLine(t *testing.T) {
	cmd := "/opt/homebrew/bin/llama-server -m ~/models/example-35b-v2-Q4_K_M.gguf -c 262144 --host 127.0.0.1 --port 9000 --log-disable"
	if got := modelFileFromCmdLine(cmd); got != "~/models/example-35b-v2-Q4_K_M.gguf" {
...
func TestSweepTargetSelection(t *testing.T) {
	...
	cases := []struct {
		...
		{target, false}, // 目标 → 保留
		{"~/models/example-35b-v2-Q4_K_M.gguf", true}, // 其它 → 清场
