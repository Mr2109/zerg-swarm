...
type OrnithAdapter struct {
	...
	Thinking    bool   // 思考开关——ornith 思考占 max_tokens（1.5 尤其明显）
	AuthToken   string
...
func (o *OrnithAdapter) Capabilities() []string {
	return []string{
		"ornith-temperature-0.8",
		"ornith-auth-token",
		"ornith-finish-words",
		"ornith-responses-api",
	}
}
...
func (o *OrnithAdapter) Init(cfg map[string]interface{}) error {
	...
	if v, ok := cfg["temperature"]; ok {
		...
		case int:
			o.Temperature = float64(t)
		default:
			return fmt.Errorf("ornith: temperature must be number, got %T", v)
		}
	}
	...
	if v, ok := cfg["top_p"]; ok {
		...
		case int:
			o.TopP = float64(t)
		default:
			return fmt.Errorf("ornith: top_p must be number, got %T", v)
		}
	}
	...
	if v, ok := cfg["top_k"]; ok {
		...
		case int:
			o.TopK = t
		default:
			return fmt.Errorf("ornith: top_k must be number, got %T", v)
		}
	}
	...
	if v, ok := cfg["min_p"]; ok {
		...
		case int:
			o.MinP = float64(t)
		default:
			return fmt.Errorf("ornith: min_p must be number, got %T", v)
		}
	}
	...
	if v, ok := cfg["max_tokens"]; ok {
		...
		case int:
			o.MaxTokens = t
		default:
			return fmt.Errorf("ornith: max_tokens must be number, got %T", v)
		}
	}
	...
	if v, ok := cfg["timeout_sec"]; ok {
		...
		case int:
			o.TimeoutSec = t
		default:
			return fmt.Errorf("ornith: timeout_sec must be number, got %T", v)
		}
	}
	...
	if v, ok := cfg["auth_token"]; ok {
		if s, ok := v.(string); ok {
			...
			return fmt.Errorf("ornith: auth_token must be string")
		}
	}
...
func (o *OrnithAdapter) Start() error {
	if !o.initialized {
		return fmt.Errorf("ornith: not initialized")
	}
	o.started = true
	return nil
}
...
func (o *OrnithAdapter) Execute(input plugin.PluginInput) (plugin.PluginOutput, error) {
	if !o.started {
		return plugin.PluginOutput{}, fmt.Errorf("ornith: not started")
	}

	prompt, _ := input.Context["prompt"].(string)
	if prompt == "" {
		return plugin.PluginOutput{}, fmt.Errorf("ornith: prompt is empty")
	}
...
// 完成语义词表（ornith 散落的完成词检测——从 loop.go 搬入）

// DefaultFinishWords 返回 ornith 默认完成语义词表
...
func (o *OrnithAdapter) OptionSchema() []plugin.OptionDef {
	return []plugin.OptionDef{
		...
		{Key: "max_tokens", Type: "number", Value: o.MaxTokens, Desc: "输出上限——ornith-1.5 思考占 token，小了 reasoning 挤掉内容/tool_calls（32768 装 reasoning+content）", Min: 256, Max: 131072},
		{Key: "timeout_sec", Type: "number", Value: o.TimeoutSec, Desc: "请求超时（秒）——ornith 推理慢（31tok/s）留 120s 余量", Min: 10, Max: 600},
		{Key: "format", Type: "enum", Value: o.Format, Desc: "请求格式——chat=聊天补全 / responses=新版 API", Options: []string{"chat", "responses"}},
		{Key: "thinking", Type: "bool", Value: o.Thinking, Desc: "思考开关——ornith 思考占 max_tokens（1.5 尤其明显——占满导致 content 空/tool_calls 丢）"},
		{Key: "use_responses_api", Type: "bool", Value: o.UseResponsesAPI, Desc: "走 responses API 格式（网关 /v1/responses 路径）"},
		{Key: "gateway_url", Type: "string", Value: o.GatewayURL, Desc: "网关地址——ornith 模型服务所在（http://127.0.0.1:8082）"},
