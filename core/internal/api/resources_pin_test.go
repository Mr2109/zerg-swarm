...
func TestResourcePin_TTLRequired(t *testing.T) {
	...
	machineWithResident(t, h, "x3", resources.ResidentEntry{Alias: "example-35b-v2", State: "ready", Managed: true})
	r, fake := newPinTestRouterAndFake(h)

	for _, ttl := range []string{"0", "-1", "-600"} {
		body := `{"host":"x3","model":"example-35b-v2","ttl_s":` + ttl + `}`
...
func TestResourcePin_SuccessCallsControllerOnce(t *testing.T) {
	...
	machineWithResident(t, h, "x3",
		resources.ResidentEntry{Digest: "sha256:x", Alias: "example-35b-v2", State: "ready", Managed: true})
	r, fake := newPinTestRouterAndFake(h)

	w := doResReq(t, r, http.MethodPost, "/api/resources/pin", "test-token",
		`{"host":"x3","model":"example-35b-v2","ttl_s":600}`)
	if w.Code != http.StatusOK {
		t.Fatalf("应 200，实得 %d body=%s", w.Code, w.Body.String())
	}
	if len(fake.pinCalls) != 1 || fake.pinCalls[0] != "x3/example-35b-v2" {
...
func TestResourcePin_NotWiredReturns503(t *testing.T) {
	...
	w := doResReq(t, r, http.MethodPost, "/api/resources/pin", "test-token",
		`{"host":"x3","model":"example-35b-v2","ttl_s":60}`)
...
func TestSubEndPinController_ForwardsPin(t *testing.T) {
	...
	if err := c.PinResource("x3", "example-35b-v2", 600); err != nil {
		t.Fatalf("转发应成功：%v", err)
	}
	if gotPath != "/pin" || gotModel != "example-35b-v2" || gotTTL != 600 || gotToken != "tok" {
...
func TestSubEndPinController_NoAddressForLocal(t *testing.T) {
	...
	if err := c.PinResource("local", "example-35b-v2", 60); err == nil {
