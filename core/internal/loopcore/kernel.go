...
type Deps struct {
	...
	// ── B 项⑥（B6）成员产出引用命中校验（接线与口径见 output_claims.go）──
	// OutputClaims —— 交付轮给出的**引用/断言**（{主张, 依据=回执ID, 片段}），在产出的那一刻逐条过既有裁决。
	// 空 = 本轮**没有引用载体** ⇒ 产出口不判引用、行为与接线前逐字节一致（现网全部如此 ⇒ 零回归）。
	// 为什么由调用方给、而不是从产出正文里猜格式：引用块格式由**受控解码/合成工具**保证
	// （设计稿 v1.2.2 §第1步/§Q5），该载体尚未落地 ⇒ 本批**不发明**一套格式（待定项见文件头）。
	OutputClaims []Claim
